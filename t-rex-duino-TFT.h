...
extern ST7789DinoDisplay lcd; // defined further below; forward-declared so buttonPressed() can use it

inline bool buttonPressed() {
  return digitalRead(JUMP_BUTTON_PIN) == HIGH || lcd.touchPressed();
}
...
void gameLoop() {
  ...
  // static sprites
  const Sprite gameOverSprite(&game_overver_bm, {4, 1});
  const Sprite restartIconSprite(&restart_icon_bm, {55, 25});
  Sprite heartsSprite(&hearts_5x_bm, {95, 10});
  ...
  while (1) {
    ...
    if (settings_bootButtonPressed()) {
      PLAY(SND_MENU_BACK);
      settings_showMenu(lcd);
      clearTouchState();
      prvT = millis(); // avoid a big frame-time jump right after the menu
    }

    // render cycle
    while (1) {
      // score (top-right during play, below the restart icon after game over)
      Renderer::renderNumber(bitCanvas, gameOver ? Point2Di8{49, 44} : Point2Di8{95, 2}, score);
...
void spalshScreen() {
  // Native, full-resolution splash screen (crisp text, no upscaled
  // low-res bitmap) instead of the original memcpy_P/fillScreen loop.
  lcd.drawNativeSplash();
  ...
  while (1) {
    ...
    if (settings_bootButtonPressed()) {
      PLAY(SND_MENU_BACK);
      settings_showMenu(lcd);
      lcd.drawNativeSplash(); // redraw splash after leaving settings
      clearTouchState();
      continue;
    }
